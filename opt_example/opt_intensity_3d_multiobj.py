...
from configs.intensity_3d_multiobj import make_config
...
def main():
    ...
    objects = [build_object(spec) for spec in cfg.objects]
    ...
    for i in range(1, cfg.optimizer.max_epoch + 1):
        ...
        plot.fom_display(objects, state, history, ws, i)

        if state.beta < cfg.quant.beta_max:
            # 进行伴随仿真（此时 beta 与正向结构一致）
            grad_eps = np.zeros_like(state.eps_opt)
            for obj in objects:
            ...
            # 梯度更新完成后统一升级，仅对下一轮生效
            quant.convergence_judgment(cfg, state, history, ws, objects[0])

            state.eps_opt = quant.quant_1bit(state.params, state, cfg)
            state.index_opt = np.sqrt(state.eps_opt)

            plot.plot_optresult(objects, history, ws, i)
        else:
            plot.plot_optresult(objects, history, ws, i)
            break
